#!/usr/bin/env python3
"""
Simple CIFAR-10 example that works without PyTorch issues
This demonstrates basic functionality before addressing PyTorch compatibility
"""
...
def explore_cifar10(data_dir="/home/numair/Distiller/Cifar10"):
    ...
    # Check all batches
    print(f"\n📁 Available files:")
    for file in sorted(cifar10_dir.glob("*")):
...
def create_simple_distiller_example():
    """Create a simple example that works without PyTorch."""
...
'''
    ...
    example_file = Path("/home/numair/Distiller/DistillerPyTorch/simple_example.py")
    ...
    print(f"✅ Created simple example at: {example_file}")
    return example_file

if __name__ == "__main__":
    print("🚀 Setting up working CIFAR-10 example...")
    
    # Explore CIFAR-10 dataset
    if explore_cifar10():
        # Create working example
        example_file = create_simple_distiller_example()
        
        print(f"\n🎉 Setup complete!")
        print(f"\nTo run the working example:")
        print(f"  python {example_file}")
        print(f"\nTo explore CIFAR-10:")
        print(f"  python -c 'from setup_working_example import explore_cifar10; explore_cifar10()'")
    else:
        print("❌ Failed to find CIFAR-10 dataset")
